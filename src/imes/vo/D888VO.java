package imes.vo;

import java.util.Date;

public class D888VO {

	private String GSDM = "";
	private String BDDM = "";
	private String BDBH = "";
	private String BDRQ = "";
	private String BMMC = ""; // 部门名称
	private String SQYH = ""; // 申請用戶
	private String BDZT = ""; // 簽核狀態
	private String BDJG = ""; // 簽核結果
	private String CCLX = ""; // 出廠類型
	private String BMBH = ""; // 部門編號
	private String CCBH = ""; // 財產編號
	private String LLBH = ""; // 領料編號
	private String TLBH = ""; // 退料編號
	private String WLBH = ""; // 物料編號
	private String DDBH = ""; // 訂單編號
	private String GYSM = ""; // 供應商名
	private String BDGZ = ""; // 跟縱
	private double ZJE = 0d;  // 總金額
	private String CCNR = ""; // 出廠內容
	private String BDFD = ""; // 签呈附件
	private String QHYH = ""; // 签核用户
	private Date QHSJ = null; // 签核时间
	private String JLYH = ""; // 
	private Date JLSJ = null; //
	private String GXYH = ""; //
	private Date GXSJ = null; //
	private String DYZT = ""; // 打印状态
	public String getGSDM() {
		return GSDM;
	}
	public void setGSDM(String gSDM) {
		GSDM = gSDM;
	}
	public String getBDDM() {
		return BDDM;
	}
	public void setBDDM(String bDDM) {
		BDDM = bDDM;
	}
	public String getBDBH() {
		return BDBH;
	}
	public void setBDBH(String bDBH) {
		BDBH = bDBH;
	}
	public String getBDRQ() {
		return BDRQ;
	}
	public void setBDRQ(String bDRQ) {
		BDRQ = bDRQ;
	}
	public String getBMMC() {
		return BMMC;
	}
	public void setBMMC(String bMMC) {
		BMMC = bMMC;
	}
	public String getSQYH() {
		return SQYH;
	}
	public void setSQYH(String sQYH) {
		SQYH = sQYH;
	}
	public String getCCLX() {
		return CCLX;
	}
	public void setCCLX(String cCLX) {
		CCLX = cCLX;
	}
	public String getBMBH() {
		return BMBH;
	}
	public void setBMBH(String bMBH) {
		BMBH = bMBH;
	}
	public String getCCBH() {
		return CCBH;
	}
	public void setCCBH(String cCBH) {
		CCBH = cCBH;
	}
	public String getLLBH() {
		return LLBH;
	}
	public void setLLBH(String lLBH) {
		LLBH = lLBH;
	}
	public String getTLBH() {
		return TLBH;
	}
	public void setTLBH(String tLBH) {
		TLBH = tLBH;
	}
	public String getWLBH() {
		return WLBH;
	}
	public void setWLBH(String wLBH) {
		WLBH = wLBH;
	}
	public String getDDBH() {
		return DDBH;
	}
	public void setDDBH(String dDBH) {
		DDBH = dDBH;
	}
	public String getGYSM() {
		return GYSM;
	}
	public void setGYSM(String gYSM) {
		GYSM = gYSM;
	}
	public String getBDGZ() {
		return BDGZ;
	}
	public void setBDGZ(String bDGZ) {
		BDGZ = bDGZ;
	}
	public double getZJE() {
		return ZJE;
	}
	public void setZJE(double zJE) {
		ZJE = zJE;
	}
	public String getCCNR() {
		return CCNR;
	}
	public void setCCNR(String cCNR) {
		CCNR = cCNR;
	}
	public String getBDFD() {
		return BDFD;
	}
	public void setBDFD(String bDFD) {
		BDFD = bDFD;
	}
	public String getQHYH() {
		return QHYH;
	}
	public void setQHYH(String qHYH) {
		QHYH = qHYH;
	}
	public Date getQHSJ() {
		return QHSJ;
	}
	public void setQHSJ(Date qHSJ) {
		QHSJ = qHSJ;
	}
	public String getJLYH() {
		return JLYH;
	}
	public void setJLYH(String jLYH) {
		JLYH = jLYH;
	}
	public Date getJLSJ() {
		return JLSJ;
	}
	public void setJLSJ(Date jLSJ) {
		JLSJ = jLSJ;
	}
	public String getGXYH() {
		return GXYH;
	}
	public void setGXYH(String gXYH) {
		GXYH = gXYH;
	}
	public Date getGXSJ() {
		return GXSJ;
	}
	public void setGXSJ(Date gXSJ) {
		GXSJ = gXSJ;
	}
	public String getBDZT() {
		return BDZT;
	}
	public void setBDZT(String bDZT) {
		BDZT = bDZT;
	}
	public String getBDJG() {
		return BDJG;
	}
	public void setBDJG(String bDJG) {
		BDJG = bDJG;
	}
	public String getDYZT() {
		return DYZT;
	}
	public void setDYZT(String dYZT) {
		DYZT = dYZT;
	}
}
