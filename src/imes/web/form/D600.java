...
import imes.vo.D600VO;

@WebServlet({ "/D600", "/D600/*" })
public class D600 extends HttpController {
		...
		try {
			...
			if (action2 != null && action2.equals("QianHe")) {
				...
				if(req.getParameter("BZDM")!= null && !"".equals(req.getParameter("BZDM"))){
					...
					resp.sendRedirect("/iMes/D600/EDIT?GSDM="+req.getParameter("GSDM")+"&BDDM="+req.getParameter("BDDM")+"&BDBH="+req.getParameter("BDBH")+"&HQYH="+getUid(req)+"");
	...
	private void doAdminListGetAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String sql = "SELECT GSDM,BDDM,BDBH,SQYH,BDRQ,BDZT,BDJG,QCLX,BMMC,WJSX,WJJM,RDYH,SDYH,QCAY,QCNR1,BDFD,QHYH,QHSJ,JLYH,JLSJ,GXYH,GXSJ,DYZT,DYYH " +
				" FROM D600H WHERE 1=1 "; //
		...
		String qcay = req.getParameter("QCAY");
		
		if(qcay != null && !"".equals(qcay)){
			sql += " AND QCAY = '"+qcay+"' ";
		}
		...
		req.setAttribute("D600sqlrst", sql);
		
		List<HashMap<String, Object>> list = getList(sql);

		req.setAttribute("list", list);
		
		getServletContext().getRequestDispatcher("/WEB-INF/pages/form/D600.jsp").forward(req, resp);
	}
	...
	private void doAllowListGetAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		sql += " SELECT BDBH FROM D600H WHERE 1=1 ";
		sql += Helper.sqlParams(req, "GSDM", "STRGSDM", "ENDGSDM");
		sql += Helper.sqlParams(req, "BDBH", "STRBDBH", "ENDBDBH");
	...
	private void doListGetD600HeJunAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		sql += " OR BDBH IN ( SELECT BDBH FROM D600H WHERE 1=1 AND SQYH = '"+getUid(req)+"' ) ";
		...
		sql += " OR BDBH IN ( SELECT BDBH FROM D600H WHERE 1=1 AND BDJG ='Y' AND QHYH = '"+getUid(req)+"' )  ";
		...
		sql += " ORDER BY GSDM,BDBH";System.out.println("doListGetD600HAction sql : "+sql);
	...
	private void doCreateGetAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		getServletContext().getRequestDispatcher("/WEB-INF/pages/form/D600New.jsp").forward(req, resp);
	...
	private void doADDQHGetAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String sql = "SELECT GSDM,BDDM,BDBH,SQYH,BDRQ,BDZT,BDJG,QCLX,BMMC,WJSX,WJJM,RDYH,SDYH,QCAY,QCNR,BDFD,QHYH,QHSJ,JLYH,JLSJ,GXYH,GXSJ,DYZT,DYYH " +
				" FROM D600H WHERE 1=1 AND GSDM ='"+gsdm+"' AND BDDM ='D600' AND BDBH ='"+bdbh+"' ";
		...
		req.setAttribute("D600", d);
		
		getServletContext().getRequestDispatcher("/WEB-INF/pages/form/D600ADDQHR.jsp").forward(req, resp);
	...
	private void doEditGetAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String sq_user = "SELECT * FROM D600H WHERE GSDM = '"+gsdm+"' AND BDDM = '"+bddm+"' AND BDBH = '"+bdbh+"' AND SQYH = '"+getUid(req)+"'";
		...
		String url = "/iMes/D600/EDIT?juid=" + getUid(req)+"&GSDM="+d.getGSDM()+"&BDDM="+d.getBDDM()+"&BDBH="+d.getBDBH()+"";
		...
		if(qcgly==true){
			String url1 = "/iMes/D600/EDIT?juid=" + getUid(req)+"&GSDM="+d.getGSDM()+"&BDDM="+d.getBDDM()+"&BDBH="+d.getBDBH()+"";
			req.getSession().setAttribute("URL", url1);
			getServletContext().getRequestDispatcher("/WEB-INF/pages/form/D600QHR.jsp").forward(req, resp);
		}else{
			getServletContext().getRequestDispatcher("/WEB-INF/pages/form/D600UPDATE.jsp").forward(req, resp);
		}
	}

	private void doUpdateGetAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		D600VO hd = new D600VO();
		...
		String rdyh = req.getParameter("RDYH");
		if(rdyh != null && ",".equals(rdyh.substring(rdyh.length()-1,rdyh.length()))){
			rdyh = rdyh.substring(0,rdyh.length()-1);
		}
		hd.setRDYH(rdyh);
		String sdyh = req.getParameter("SDYH");
		if(sdyh != null && ",".equals(sdyh.substring(sdyh.length()-1,sdyh.length()))){
			sdyh = sdyh.substring(0,sdyh.length()-1);
		}
		hd.setSDYH(sdyh);
		//System.out.println("D600.java update");
		updateDb(hd);
		updateClob2(hd);
		
		String sql = "SELECT GSDM,BDDM,BDBH,SQYH,BDRQ,BDZT,BDJG,QCLX,BMMC,WJSX,WJJM,RDYH,SDYH,QCAY,QCNR,BDFD,QHYH,QHSJ,JLYH,JLSJ,GXYH,GXSJ,DYZT,DYYH " +
				" FROM D600H WHERE 1=1 AND GSDM ='"+req.getParameter("GSDM")+"' AND BDDM ='D600' AND BDBH ='"+req.getParameter("BDBH")+"' ";
		...
		String url = "/iMes/D600/UPDATE?juid=" + getUid(req)+"&GSDM="+hd.getGSDM()+"&BDDM="+hd.getBDDM()+"&BDBH="+hd.getBDBH()+"";
		req.getSession().setAttribute("URL", url);
	...
	private void doSaveGetAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		hd.setGSDM(req.getParameter("GSDM"));
		hd.setBDDM("D600");
		hd.setBDBH(getBDBH(req.getParameter("GSDM"), "D600", Helper.fmtDate(new Date(), "yyyy")));
		...
		String sql = "SELECT GSDM,BDDM,BDBH,SQYH,BDRQ,QCLX,BMMC,WJSX,WJJM,RDYH,SDYH,QCAY,QCNR,BDFD,QHYH,QHSJ,JLYH,JLSJ,GXYH,GXSJ " +
				" FROM D600H WHERE 1=1 ";
		...
		req.setAttribute("D600", hd);
		req.setAttribute("BDLCS", BDLCS);
		req.setAttribute("QHKS", QHKS);
		
		String url = "/iMes/D600/EDIT?juid=" + getUid(req)+"&GSDM="+hd.getGSDM()+"&BDDM="+hd.getBDDM()+"&BDBH="+hd.getBDBH()+"";
		req.getSession().setAttribute("URL", url);
		getServletContext().getRequestDispatcher("/WEB-INF/pages/form/D600UPDATE.jsp").forward(req, resp);
	}
	...
	private void actionSubmitQH(HttpServletRequest req, HttpServletResponse resp) {
		...
		try{
			...
			String sql = "UPDATE D600H SET BDZT='0', BDJG='0' WHERE GSDM=? AND BDDM=? AND BDBH=?";
			...
			String sql_d600 = "SELECT GSDM,BDDM,BDBH,SQYH,BDRQ,BDZT,BDJG,QCLX,BMMC,WJSX,WJJM,RDYH,SDYH,QCAY,QCNR,BDFD,QHYH,QHSJ,JLYH,JLSJ,GXYH,GXSJ,DYZT,DYYH " +
					" FROM D600H WHERE 1=1 AND GSDM ='"+req.getParameter("GSDM")+"' AND BDDM ='D600' AND BDBH ='"+req.getParameter("BDBH")+"' ";
					
			D600VO d = new D600VO();
			d = getVO(sql_d600,gsdm,bddm,bdbh);
			
			//签核RD人员
			bzdm = 300;
			zwdm = "RDD600";
			String [] rdyh = null;
			if(d.getRDYH().contains(","))
				rdyh = d.getRDYH().split(",");
			if(d.getRDYH().contains(";"))
				rdyh = d.getRDYH().split(";");
			if(rdyh != null){
				for(int rdy=0;rdy < rdyh.length;rdy++){
					bdlc = addQHE(gsdm,bddm,bdbh,qhzt,bzdm,1000,zwdm,"I","N",sqyh,rdyh[rdy]);
					q.add(bdlc);
					bzdm = bzdm + 10;
				}
			}else{
				bdlc = addQHE(gsdm,bddm,bdbh,qhzt,bzdm,1000,zwdm,"I","N",sqyh,d.getRDYH());
				q.add(bdlc);
			}
			...
			zwdm = "SDD600";
			String [] sdyh = null;
			if(d.getSDYH().contains(","))
				sdyh = d.getSDYH().split(",");
			if(d.getSDYH().contains(";"))
				sdyh = d.getSDYH().split(";");
			if(sdyh !=null ){
				for(int sdy=0;sdy < sdyh.length;sdy++){
					bdlc = addQHE(gsdm,bddm,bdbh,qhzt,bzdm,1000,zwdm,"I","N",sqyh,sdyh[sdy]);
					q.add(bdlc);
					bzdm = bzdm + 10;
				}
			}else{
				bdlc = addQHE(gsdm,bddm,bdbh,qhzt,bzdm,1000,zwdm,"I","N",sqyh,d.getSDYH());
				q.add(bdlc);
			}
			...
			con.commit();
			resp.sendRedirect("/iMes/D600/LIST?STRGSDM=" + req.getParameter("GSDM") + "&STRBDDM=" + req.getParameter("BDDM")
	...
	private void actionSubmitQHGLY(HttpServletRequest req, HttpServletResponse resp) {
		...
		D600VO	d = getVO(sql,gsdm,bddm,bdbh);
	...
	private void actionExportExcel(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			Label labelCF10 = new Label(9, 0, "RD人员");
			Label labelCF11 = new Label(10, 0, "SD人员");
			Label labelCF12 = new Label(11, 0, "机种名称");
			Label labelCF13 = new Label(12, 0, "流程号");
			Label labelCF14 = new Label(13, 0, "");
			Label labelCF15 = new Label(14, 0, "签核状态");
			Label labelCF16 = new Label(15, 0, "应签核人");
			Label labelCF17 = new Label(16, 0, "实签核人");
			Label labelCF18 = new Label(17, 0, "签核时间");
			Label labelCF19 = new Label(18, 0, "签核结果");
			Label labelCF20 = new Label(19, 0, "签核内容");
			ws.addCell(labelCF1);
			ws.addCell(labelCF2);
			ws.addCell(labelCF3);
			ws.addCell(labelCF4);
			...
			if(sqlrst!=null && sqlrst.length()>0){
				...
				String sql = "SELECT D600.GSDM,D600.BDDM,D600.BDBH,D600.SQYH,"
						+"BDRQ,BDZT,BDJG,QCLX,BMMC,WJSX,WJJM,QCAY,QCNR1,BDFD," 
						+"D600.QHYH,D600.QHSJ,JLYH,D600.JLSJ,GXYH,GXSJ,DYZT,DYYH," 
						+"QH.YSYH,QH.QHYH QHYH2,QH.QHSJ QHSJ2,QH.QHJG,QH.QHNR,"
						+"QH.LCLX,QH.BZDM,QH.QHZT "
						+" FROM "
						+" ( "+sqlrst+" ) "
						+" D600 "
						+" JOIN QH_BDLC QH ON D600.GSDM = QH.GSDM AND D600.BDDM = QH.BDDM AND D600.BDBH = QH.BDBH ";
		    	...
		    	while(rs.next()) {
					...
					Label data10 = new Label(9, sn,rs.getString("RDYH"));   
					Label data11 = new Label(10, sn,rs.getString("SDYH"));  
					Label data12 = new Label(11, sn,rs.getString("QCAY"));
					Label data13 = new Label(12, sn,rs.getString("BZDM"));
					Label data14 = new Label(13, sn,rs.getString("LCLX"));
					Label data15 = new Label(14, sn,rs.getString("QHZT"));
					Label data16 = new Label(15, sn,rs.getString("YSYH"));
					Label data17 = new Label(16, sn,rs.getString("QHYH2"));
					Label data18 = new Label(17, sn,rs.getString("QHSJ2"));   
					Label data19 = new Label(18, sn,rs.getString("QHJG"));  
					Label data20 = new Label(19, sn,rs.getString("QHNR"));
					ws.addCell(data1);
					ws.addCell(data2);
					ws.addCell(data3);
					ws.addCell(data4);
	...
	private void doPrintGetAction(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			getServletContext().getRequestDispatcher("/WEB-INF/pages/form/D600Print.jsp").forward(req, resp);
	...
	public int updateDb(D600VO D600VO) {
		...
		String sql = "UPDATE D600H SET BMMC=?,SQYH=?,QCLX=?,RDYH=?,QCAY=?,SDYH=? WHERE GSDM=? AND BDDM=? AND BDBH=? ";

		try {
			conn = getConnection();
			//System.out.println("D600.java updateDb sql:"+sql);
			pstm1 = conn.prepareStatement(sql);
			
			pstm1.setString(1, D600VO.getBMMC());//System.out.println("D600.java updateDb BMMC:"+D600VO.getBMMC());
			pstm1.setString(2, D600VO.getSQYH());//System.out.println("D600.java updateDb SQYH:"+D600VO.getSQYH());
			pstm1.setString(3, D600VO.getQCLX());//System.out.println("D600.java updateDb QCLX:"+D600VO.getQCLX());
			pstm1.setString(4, D600VO.getRDYH());//System.out.println("D600.java updateDb RDYH:"+D600VO.getRDYH());
			pstm1.setString(5, D600VO.getQCAY());//System.out.println("D600.java updateDb QCAY:"+D600VO.getQCAY());
			pstm1.setString(6, D600VO.getSDYH());//System.out.println("D600.java updateDb SDYH:"+D600VO.getSDYH());
			pstm1.setString(7, D600VO.getGSDM());//System.out.println("D600.java updateDb GSDM:"+D600VO.getGSDM());
			pstm1.setString(8, D600VO.getBDDM());//System.out.println("D600.java updateDb BDDM:"+D600VO.getBDDM());
			pstm1.setString(9, D600VO.getBDBH());//System.out.println("D600.java updateDb BDBH:"+D600VO.getBDBH());
	...
	public boolean updateClob3(D600VO D600VO) {
        ...
        String sql = "SELECT * FROM D600H " +
        	" WHERE GSDM='"+D600VO.getGSDM()+"' AND BDDM='"+D600VO.getBDDM()+"'" +
    		" AND BDBH='"+D600VO.getBDBH()+"' for update ";
        ...
        try {
            ...
            pw.write(D600VO.getQCNR());
  
            pw.flush();  
            pstm1.close();
              
            sql = "UPDATE D600H SET QCNR1=? " +
            	  " WHERE GSDM='"+D600VO.getGSDM()+"' AND BDDM='"+D600VO.getBDDM()+"'" +
    			  " AND BDBH='"+D600VO.getBDBH()+"' "; 
	...
	public boolean updateClob2(D600VO D600VO) {
        ...
        try {
            ...
            if(rst1.next()){
                ...
                clob.putChars(1, D600VO.getQCNR().toCharArray());
	...
	public D600VO getVO(String sql,String GSDM,String BDDM,String BDBH) {
		...
		try {
			...
			while(rst.next()){
				...
				d.setRDYH(rst.getString("RDYH"));
				d.setSDYH(rst.getString("SDYH"));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public D600VO find(String GSDM,String BDDM,String BDBH) {
		...
		D600VO D600VO = new D600VO();

		String sql = "SELECT * FROM D600H WHERE GSDM=? AND BDDM=? AND BDBH=?";
		try {
			con = getConnection();
			...
			if (rst.next()) {
				D600VO.setGSDM(rst.getString("GSDM"));
				D600VO.setBDDM(rst.getString("BDDM"));
				D600VO.setBDBH(rst.getString("BDBH"));
				D600VO.setBDRQ(rst.getString("BDRQ"));
				D600VO.setBMMC(rst.getString("BMMC"));
				D600VO.setSQYH(rst.getString("SQYH"));
				D600VO.setQCLX(rst.getString("QCLX"));
				D600VO.setWJSX(rst.getString("WJSX"));
				D600VO.setWJJM(rst.getString("WJJM"));
				D600VO.setQCAY(rst.getString("QCAY"));
				D600VO.setQCNR(readClob(GSDM,BDDM,BDBH));
				D600VO.setBDFD(rst.getString("BDFD"));
				D600VO.setQHYH(rst.getString("QHYH"));
				D600VO.setQHYH(rst.getString("QHYH"));
				D600VO.setQHSJ(rst.getTimestamp("QHSJ"));
				D600VO.setJLYH(rst.getString("JLYH"));
				D600VO.setJLSJ(rst.getTimestamp("JLSJ"));
				D600VO.setGXYH(rst.getString("GXYH"));
				D600VO.setGXSJ(rst.getTimestamp("GXSJ"));
				D600VO.setDYZT(rst.getString("DYZT"));
				D600VO.setDYYH(rst.getString("DYYH"));
				D600VO.setRDYH(rst.getString("RDYH"));
				D600VO.setSDYH(rst.getString("SDYH"));
			}
...
//System.out.println("sql bd;; "+sql);
	...
	public String getD600H_QHJG(String bdbh) {
	...
	@SuppressWarnings("deprecation")
	public boolean inserClob(D600VO D600VO) {
        ...
        String sql = "INSERT INTO D600H (GSDM,BDDM,BDBH,BDRQ,BMMC," +
        		"SQYH,QCLX,WJSX,QCAY,QCNR1,BDFD,QHYH,QHSJ,JLYH," +
        		"JLSJ,GXYH,GXSJ,WJJM,RDYH,SDYH) " +
        		"VALUES ('"+D600VO.getGSDM()+"','"+D600VO.getBDDM()+"'" +
        		",'"+D600VO.getBDBH()+"','"+D600VO.getBDRQ()+"'," +
        		"'"+D600VO.getBMMC()+"','"+D600VO.getSQYH()+"'," +
        		"'"+D600VO.getQCLX()+"','"+D600VO.getWJSX()+"'" +
        		",'"+D600VO.getQCAY()+"',empty_clob()" +
        		",'"+D600VO.getBDFD()+"','"+D600VO.getQHYH()+"'" +
        		","+Helper.toSqlDate(D600VO.getQHSJ())+",'"+D600VO.getJLYH()+"'" +
        		","+Helper.toSqlDate(D600VO.getJLSJ())+",'"+D600VO.getGXYH()+"'" +
        		","+Helper.toSqlDate(D600VO.getGXSJ())+",'"+D600VO.getWJJM()+"'" +
        		",'"+D600VO.getRDYH()+"','"+D600VO.getSDYH()+"')";
        
        try {
        	con = getConnection();
        	
            con.setAutoCommit(false);
            System.out.println("D600.java :sql="+sql);
            pstm = con.prepareStatement(sql);
    ...
    public String readClob(String GSDM,String BDDM,String BDBH){ 
        ...
        String sql = "SELECT QCNR1 FROM D600H WHERE GSDM='"+GSDM+"' AND BDDM='"+BDDM+"' AND BDBH='"+BDBH+"'";
