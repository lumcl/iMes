...
window.onload = function()
{
	...
	if ( FCKConfig.ImageUpload ) GetE('frmUpload_dir').action = FCKConfig.ImageUploadURL ;
...
function CheckUpload()
{
	...
	if ( sFile.length == 0 )
	{
		...
		alert( '请先选择一个图片文件进行上传！' ) ;
		return false ;
	} else {
		 var suffix = sFile.substring(sFile.lastIndexOf('.') + 1).toLowerCase();
		 if (suffix != 'jpg' && suffix != 'bmp' && suffix != 'jpeg' && suffix != 'png' && suffix != 'gif') {
		 	alert('请选择正确的图片文件格式进行上传！系统只支持jpg、jpeg、bmp、png和gif格式的图片！');
		 	return false;
		}
	}
