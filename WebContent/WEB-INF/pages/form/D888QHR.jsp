...
<script type="text/javascript" src="/iMes/javascript/D888QHR.js"></script>
...
<div id="tabs">
  <form action="/iMes/D888/SubmitQH" method="post" id="SBMQHFRM">
   ...
   <input type="hidden" name="BDTX" value="${D888.QCAY}">
 ...
 <div id="tab-1">
  <form action="/iMes/D888/GLYQH" method="post" id="D888QHFRM">
   <input type="hidden" name="GSDM" value="${D888.GSDM}">
   <input type="hidden" name="BDDM" value="${D888.BDDM}">
   <input type="hidden" name="BDBH" value="${D888.BDBH}">
   <input type="hidden" name="BDTX" value="${D888.QCAY}">
   ...
   <table style="width: 1050px;">
    ...
    <tbody>
     <tr>
      ...
      <td><select name="GSDM" id="GSDM">
        ...
        <option value="L921" <c:if test="${D888.GSDM == 'L921'}">selected="selected"</c:if>>L921 日本立德</option>
      </select></td>
      <th>簽呈類型</th>
      <td><select name="QCLX">
        <option value="人事" <c:if test="${D888.QCLX == '人事'}">selected="selected"</c:if>>人事</option>
        <option value="獎懲" <c:if test="${D888.QCLX == '獎懲'}">selected="selected"</c:if>>獎懲</option>
        <option value="規章" <c:if test="${D888.QCLX == '規章'}">selected="selected"</c:if>>規章</option>
        <option value="異常" <c:if test="${D888.QCLX == '異常'}">selected="selected"</c:if>>異常</option>
        <option value="一般" <c:if test="${D888.QCLX == '一般'}">selected="selected"</c:if>>一般</option>
      </select></td>
      <th colspan="2">文件屬性</th>
      <td><select name="WJSX">
       <option value="普通件" <c:if test="${D888.WJSX == '普通件'}">selected="selected"</c:if>>普通件</option>
       <option value="急件" <c:if test="${D888.WJSX == '急件'}">selected="selected"</c:if>>急件</option>
       </select></td>
      <th>機密</th>
      <td><select name="WJJM">
       <option value="非機密" <c:if test="${D888.WJJM == '非機密'}">selected="selected"</c:if>>非機密</option>
       <option value="機密" <c:if test="${D888.WJJM == '機密'}">selected="selected"</c:if>>機密</option>
       </select></td>
     </tr>
     <tr>
      <th>部門代碼</th>
    ...
    <tr>
     <th>案&nbsp;&nbsp;&nbsp;&nbsp;由</th>
     <td colspan="4"><input type="text" name="QCAY" id="QCAY" style="width:500px;" value="${D888.QCAY}"/></td>
     <th>申&nbsp;請&nbsp;人</th>
     <td><input type="text" name="SQYH" id="SQYH" size="15" value="${D888.SQYH}"/></td>
     <th>表單附檔</th>
    ...
    <tr>
     <th>簽呈內容</th>
     <td colspan="8"><textarea name="QCNR" id="QCNR" rows="150" cols="1050" >${D888.QCNR}</textarea> 
		<script type="text/javascript">  
